...
public class CRC_PatientCurrentOrdered2Confirm extends Fragment {
...
//    String userType;
	...
	public CRC_PatientCurrentOrdered2Confirm(String checkupId,String patientId) {
...
//		userType = sp.getString("userType","");
		...
		odth.start();
		view = inflater.inflate(R.layout.crc_activity_current_ordered_confirm, container, false);
		...
		view.findViewById(R.id.crcorderingconfirmbutton).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(final View v) {
                new AlertDialog.Builder(CRC_PatientCurrentOrdered2Confirm.this.getActivity()).setTitle("提示")//设置对话框标题
                        .setMessage("确认患者预约?")//设置显示的内容
                        .setPositiveButton("确定", new DialogInterface.OnClickListener() {//添加确定按钮
                            @Override
                            public void onClick(DialogInterface dialog, int which) {//确定按钮的响应事件
                                // TODO Auto-generated method stub
                                Toast.makeText(CRC_PatientCurrentOrdered2Confirm.this.getActivity(), "确认成功", 5).show();
...
//                         		mapParam.put("password", "passw0rd");
                                mapParam.put("checkupId", CRC_PatientCurrentOrdered2Confirm.this.checkupId);
                                //mapParam.put("checkupId", "1");
                                //ordertimetext.getText().toString()
                                TextView timeV=(TextView)view.findViewById(R.id.crc_ordertimetext);
                         		mapParam.put("checkupPlanDate", timeV.getText().toString());
                         		TextView comV=(TextView)view.findViewById(R.id.crc_ordercomment);
                         		mapParam.put("comments",comV.getText().toString());
                                HttpConnections httpConnections = new HttpConnections();
                         		JSONObject obj  = httpConnections.httpConnectionPut(Constants.CRC_CHECKUPCONFIRM, token,mapParam);
                         		try {
									if(Integer.parseInt(obj.get("statuCode").toString())<Constants.SUCCESSCODE){
										LinearLayout lyc = (LinearLayout)view.findViewById(R.id.crc_currentordercontainer);
										lyc.removeAllViews();
										TextView notextView = new TextView(view.getContext());
										notextView.setText( "暂无预约项目" );
		...
		view.findViewById(R.id.crcrejectbutton).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(final View v) {
                new AlertDialog.Builder(CRC_PatientCurrentOrdered2Confirm.this.getActivity()).setTitle("提示")//设置对话框标题
                        .setMessage("确认驳回患者预约?")//设置显示的内容
                        .setPositiveButton("确定", new DialogInterface.OnClickListener() {//添加确定按钮
                            @Override
                            public void onClick(DialogInterface dialog, int which) {//确定按钮的响应事件
                                // TODO Auto-generated method stub
...
//                         		mapParam.put("password", "passw0rd");
                                mapParam.put("checkupId", CRC_PatientCurrentOrdered2Confirm.this.checkupId);
                                HttpConnections httpConnections = new HttpConnections();
                         		JSONObject obj  = httpConnections.httpConnectionPut(Constants.CRC_REJECT, token,mapParam);
                         		try {
									if(Integer.parseInt(obj.get("statuCode").toString())<Constants.SUCCESSCODE){
										LinearLayout lyc = (LinearLayout)view.findViewById(R.id.crc_currentordercontainer);
										lyc.removeAllViews();
...
//			LoginActivity.this.finish();
        	...
        	TextView orderidV=(TextView)view.findViewById(R.id.crc_orderid);
        	orderidV.setText(msg.getData().getString("crc_orderid"));
        	TextView timeV=(TextView)view.findViewById(R.id.crc_ordertimetext);
        	timeV.setText(msg.getData().getString("crc_ordertimetext"));
        	TextView nameV=(TextView)view.findViewById(R.id.crc_patient);
        	nameV.setText(msg.getData().getString("crc_patientid"));
    ...
    class OrderedThread extends Thread{
        public void run() {
            ...
            Message msg = new Message();
      		HttpConnections httpConnections = new HttpConnections();
     		JSONObject obj  =httpConnections.httpConnectionGet(Constants.CHECKUP+Constants.CHECKUPID+checkupId+Constants.PATIENTID+patientId, token);
...
//    				crc_ordercomment
      	  ...
      	  CRC_PatientCurrentOrdered2Confirm.this.myHandler.sendMessage(msg);
        }
    }
