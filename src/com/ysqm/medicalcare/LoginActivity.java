...
public class LoginActivity extends Activity {
		...
		findViewById(R.id.sign_in_button).setOnClickListener(
				new View.OnClickListener() {
					...
					public void onClick(View view) {
						...
						//login
						if("".equals(userId)||"".equals(userPWD)){
							Toast.makeText(LoginActivity.this, "请输入正确的用户名和密码", 5).show();
							pd.dismiss();
						}else{
							lth = new LoginThread();
				             lth.start();
	...
	Handler myHandler = new Handler(){
        ...
        public void handleMessage(Message msg) {
        	...
        	pd.dismiss();
        	if(Integer.parseInt(msg.getData().getString("stateCode").toString())<400){
...
//     		mapParam.put("password", "passw0rd");
     		JSONObject obj  =httpConnections.httpConnectionLoginPost(Constants.LOGIN, mapParam);
      	    try {
      	    	if(Integer.parseInt(obj.get("stateCode").toString())<400){
    				...
    				JPushInterface.setAliasAndTags(getApplicationContext(), obj.getString("userId"), null, null);  
    				msg.setData(bundle);
      	    	}else{
      	    		Bundle bundle = new Bundle();
    				bundle.putString("stateCode", obj.getString("stateCode"));
