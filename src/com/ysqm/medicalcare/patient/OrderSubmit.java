...
//			}
//		});
        pd = ProgressDialog.show(OrderSubmit.this.getActivity(), "提示", "加载中，请稍后……"); 
        ordertimetext = (TextView)view.findViewById(R.id.ordertimetext);
        Calendar calendar = Calendar.getInstance();
        
        String cm = (String.valueOf(calendar.get(Calendar.MONTH)+1).length()>1) ? String.valueOf(calendar.get(Calendar.MONTH)+1) :"0"+String.valueOf(calendar.get(Calendar.MONTH)+1);
        String cd = (String.valueOf(calendar.get(Calendar.DAY_OF_MONTH)).length()>1) ? String.valueOf(calendar.get(Calendar.DAY_OF_MONTH)) :"0"+String.valueOf(calendar.get(Calendar.DAY_OF_MONTH));
        String cH = (String.valueOf(calendar.get(Calendar.HOUR_OF_DAY)).length()>1) ? String.valueOf(calendar.get(Calendar.HOUR_OF_DAY)) :"0"+String.valueOf(calendar.get(Calendar.HOUR_OF_DAY));
        String cM = (String.valueOf(calendar.get(Calendar.MINUTE)).length()>1) ? String.valueOf(calendar.get(Calendar.MINUTE)) :"0"+String.valueOf(calendar.get(Calendar.MINUTE));
         
        ordertimetext.setText(calendar.get(Calendar.YEAR) + "-" +
        		cm + "-" +
        		cd + " " +cH+":"+cM);
        LinearLayout ordertimebutton = (LinearLayout)view.findViewById(R.id.ordertimebutton);
        ...
        ordertimebutton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                ...
                ScreenInfo screenInfo = new ScreenInfo(OrderSubmit.this.getActivity());
                ...
                new AlertDialog.Builder(view.getContext())
                        ...
                        .setPositiveButton("确定", new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                ordertimetext.setText(wheelMain.getTime());
                            }
                        })
        ...
        view.findViewById(R.id.ordersubmitbutton).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(final View v) {
                new AlertDialog.Builder(OrderSubmit.this.getActivity()).setTitle("提示")//设置对话框标题
                        .setMessage("确认提交本次预约?")//设置显示的内容
                        .setPositiveButton("确定", new DialogInterface.OnClickListener() {//添加确定按钮
                            @Override
                            public void onClick(DialogInterface dialog, int which) {//确定按钮的响应事件
                                // TODO Auto-generated method stub
                                Toast.makeText(OrderSubmit.this.getActivity(), "预约成功", 5).show();
...
//                         		mapParam.put("password", "passw0rd");
                                mapParam.put("checkupId", ((TextView)view.findViewById(R.id.p_orderid)).getText().toString());
                                //mapParam.put("checkupId", "1");
                                //ordertimetext.getText().toString()
                         		mapParam.put("checkupPlanDate", ordertimetext.getText().toString());
//                         		mapParam.put("state", "ordered");
                                HttpConnections httpConnections = new HttpConnections();
                         		JSONObject obj  = httpConnections.httpConnectionPut(Constants.P_SUBMITORDER, token,mapParam);
                         		try {
...
//        	timeV.setText(msg.getData().getString("p_ordertimetext"));
        	//System.out.println("+++s+"+msg.getData().isEmpty()+"+ssss");
        	if(msg.getData().isEmpty()){
        		...
        		TextView ordertimereV=(TextView)view.findViewById(R.id.ordertimeregion);
        		String ob=msg.getData().getString("p_winstart").toString().substring(0,10);
        		String oe=msg.getData().getString("p_winend").substring(0,10);
            	ordertimereV.setText("根据随访规则请在"+ob+"至"+oe+"之间参加本次随访");
        		TextView orderidV=(TextView)view.findViewById(R.id.p_orderid);
...
//            	TextView mentionV=(TextView)view.findViewById(R.id.p_ordermentioncontainer);
    ...
    class OrderingThread extends Thread{
        public void run() {
            ...
            Message msg = new Message();
      		HttpConnections httpConnections = new HttpConnections();
     		JSONObject obj  =httpConnections.httpConnectionGet(Constants.P_CURRENTORDER+URLEncoder.encode(Constants.P_CURRENTORDER_STATE), token);
      	    ...
      	    try {
      	    	...
      	    	if(obj.getJSONArray("data").length()>0){
    				...
    				Bundle bundle = new Bundle();
    				bundle.putString("p_orderid", CurrentOrderObj.getString("checkupId"));
    				bundle.putString("p_winstart", CurrentOrderObj.getString("winStart"));
    				bundle.putString("p_winend", CurrentOrderObj.getString("winEnd"));
